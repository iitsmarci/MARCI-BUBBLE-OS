    ...
    <Bubble
      ...
      meta={
        <span className="live-mini">
          <i /> {feed.kind === 'connected' ? 'ICAL' : 'LOCAL'}
        </span>
      }
    >
      {isLoading && events.length === 0 ? (
        ...
        <div className="empty-state">
          ...
          <div>
            ...
            <p>La tua agenda per oggi è libera.</p>
          </div>
        </div>
      ) : (
        <>
          <p className="next-label">{feed.kind === 'connected' ? 'PROSSIMI EVENTI' : 'OGGI IN AGENDA'}</p>
      ...
      <footer>
        {feed.kind === 'connected'
          ? `Apple Calendar · ${events.length} eventi in programma`
